...
def capturar_dataset_persona():
    """Captura múltiples fotos de una persona para dataset"""
    ...
    print("\nInstrucciones:")
    print("  • Mantén el rostro de frente")
    print("  • Buena iluminación frontal")
    print("  • No muevas la cabeza bruscamente")
    print("  • Variaciones: con/sin sonrisa, ligeros ángulos")
    ...
    while fotos_capturadas < objetivo:
        ...
        if len(faces) > 0:
            ...
            cv2.putText(frame, f"Calidad: {puntuacion:.0f}/100",
                       (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color_bbox, 2)
        
        # Información en pantalla
        cv2.putText(frame, f"Fotos: {fotos_capturadas}/{objetivo}",
                   (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        
        cv2.putText(frame, mensaje_calidad,
                   (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_mensaje, 2)
        
        if puntuacion >= 50:
            cv2.putText(frame, "ESPACIO para capturar",
    ...
    if fotos_capturadas == objetivo:
        ...
        print(f"\n⚠️  Solo se capturaron {fotos_capturadas}/{objetivo} fotos")
